        ...
        {isCollapsed ? (
          <>
            ...
            {!mobile && (
              <button
                ...
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-all duration-200 opacity-0 group-hover:opacity-100"
          ...
          <>
            ...
            {!mobile && (
              <button
                onClick={() => setCollapsed(!collapsed)}
                className="ml-auto w-11 h-11 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-xl transition-all duration-200"
            ...
            {mobile && (
              <button onClick={() => setSidebarOpen(false)} className="ml-auto p-3 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-all">
                <X size={20} />
              </button>
            )}
          </>
        ...
        <div className={`flex flex-col items-center gap-3 ${isCollapsed ? '' : 'px-4'}`}>
          <div className={`flex items-center ${isCollapsed ? 'flex-col gap-2' : 'gap-3 w-full'}`}>
            <button
              ...
              className="w-11 h-11 bg-white border border-gray-200 rounded-xl flex items-center justify-center overflow-hidden text-indigo-700 font-black text-xs flex-shrink-0 hover:border-indigo-400 transition-all shadow-sm group"
            ...
            {!isCollapsed && (
              <>
                ...
                <button onClick={handleLogout} title="Sign out"
                  className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all active:scale-90">
                  <LogOut size={16} />
                </button>
              </>
            )}
